...
Since all changes are saved, no business data is lost. Each operation results in the event stored in the database. Keeping track of each operation enables extended auditing and diagnostics capabilities (both technically and business-wise). What's more, as events contains the business context, it allows business wide analysis and reporting.
